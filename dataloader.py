...
class DatasetVAL(ParentDataset):
    """ F0 and Loudness dataset."""

    def __init__(self):
        if COMPUTE_CACHE or not os.path.isdir(FRAGMENT_CACHE_PATH_VAL):
            compute_cache()
        self.nb_frags = len(os.listdir(FRAGMENT_CACHE_PATH_VAL))

    def __len__(self):
        return self.nb_frags

    def __getitem__(self, idx):
        frag_path = os.path.join(FRAGMENT_CACHE_PATH_VAL,
                                 FRAGMENT_CACHE_PATTERN.format(idx))      
        frag = torch.load(frag_path)
        return frag
...
class DatasetTEST(ParentDataset):
    """ F0 and Loudness dataset."""

    def __init__(self):
        if COMPUTE_CACHE or not os.path.isdir(FRAGMENT_CACHE_PATH_TEST):
            compute_cache()
        self.nb_frags = len(os.listdir(FRAGMENT_CACHE_PATH_TEST))

    def __len__(self):
        return self.nb_frags

    def __getitem__(self, idx):
        frag_path = os.path.join(FRAGMENT_CACHE_PATH_TEST,
                                 FRAGMENT_CACHE_PATTERN.format(idx))      
        frag = torch.load(frag_path)
        return frag
...
def compute_cache():
    ...
    for audio_filename, f0_filename in zip(audio_filenames, f0_filenames):
        ...
        audio, sr = torchaudio.load('./Inputs/Sax/Audio/'+audio_filename)  
        mel_specgram = torchaudio.transforms.MelSpectrogram(sr,n_fft=2048,f_min =0,f_max=8000,
                                                            win_length=None,hop_length=None, n_mels=80,pad=0)(audio)
        fo_full2=librosa.yin(audio.numpy()[0],fmin=165,fmax=255,sr=sr,frame_length=2048,win_length=None,
                             hop_length=None,trough_threshold=0.1,center=True,pad_mode='reflect')
